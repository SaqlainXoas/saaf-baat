...
class TestRawArticleModel:
    ...
    def test_empty_main_text_rejected(self):
        """main_text must be non-empty."""
        with pytest.raises(ValidationError):
            RawArticle(
                source="dawn",
                url="https://dawn.com/article",
    ...
    def test_headline_only_article_is_accepted(self):
        """
        Tier B corroboration records carry a headline and no body until they
        are selected, so a short main_text is valid and labelled as such.
        """
        article = RawArticle(
            source="geo",
            url="https://www.geo.tv/latest/1-test",
            headline="Cabinet approves gas tariff revision",
            main_text="Cabinet approves gas tariff revision",
